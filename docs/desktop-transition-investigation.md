...
- **Not the `maybeTakeSnapshot` cadence.** That hook fires only on page saves from `PageRoute`, so map-only, timeline-only and manuscript-only sessions would never have marked the world dirty. The mirror polls instead, so there is no dirty flag and no edit path that can forget to opt in.
  ...
  **It is still not complete, and the gap is by construction:** an in-place edit to a row on those nine tables is invisible between polls, and `maps`/`calendars` notice an add but not an edit (their `createdAt` index doesn't move). Worse, the quiet window means a steady typist slides `lastChangeAt` forward on every poll and gets **no write at all** for the whole session — [#233](https://github.com/KantikPotatoe/Lore-Codex/issues/233), open, with a `MIRROR_MAX_STALE_MS` ceiling as the proposed fix. `flushWorldMirror()`'s unconditional write on close is the deliberate backstop for all of it, and is what bounds a long session today.
