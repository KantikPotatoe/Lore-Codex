...
- **Not the `maybeTakeSnapshot` cadence.** That hook fires only on page saves from `PageRoute`, so map-only, timeline-only and manuscript-only sessions would never have marked the world dirty. The mirror polls instead, so there is no dirty flag and no edit path that can forget to opt in.
  ...
  **What it polls, precisely** — an earlier revision of this section said `latestChangeTime()`, "six indexed boundary reads that see every table". Both halves were wrong, in the reassuring direction. `latestChangeTime()` (`src/backup.ts`) reads an indexed `updatedAt`/`createdAt` boundary on **6 of the 15 tables `exportAll()` writes**; the other nine (pins, regions, templates, docLinks, books, chapters, plotlines, beats, meta) carry no timestamp field at all, so a pin-only, plotline-only or settings-only session left it flat. It stays as-is because `BackupBanner` and `backupOnExit` depend on exactly that shape. The mirror therefore polls its own `mirrorChangeTime()` (`src/worldMirrorSync.ts`), which combines those six reads with a `count()` on each of the other nine, so an add or a delete registers even with no timestamp to read.

  **It is still not complete, and the gap is by construction:** an in-place edit to a row on those nine tables is invisible between polls, and `maps`/`calendars` notice an add but not an edit (their `createdAt` index doesn't move). [#233](https://github.com/KantikPotatoe/Lore-Codex/issues/233) is closed: a `MIRROR_MAX_STALE_MS` ceiling (10 minutes) now overrides the quiet window once changes have gone unmirrored that long — never the interval floor, which is checked on every path — so a steady typist's worst-case unmirrored window is bounded at ~10.5 minutes (the ceiling plus one poll interval) instead of the whole session. `flushWorldMirror()`'s unconditional write on close is still the deliberate backstop for the two blind spots above, but it is no longer what bounds a long session — the ceiling is.
