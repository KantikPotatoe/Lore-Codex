...
- **Repository seam (#186, complete):** `src/db/repositories.ts` defines five interfaces — `PageRepository`, `MapRepository`, `TemplateRepository`, `CalendarRepository`, `ManuscriptRepository` — with Dexie-backed implementations. Its header is explicit that this is **not** a portability layer: the seam exists so the UI has exactly one idiom for reaching data (routes/components call a repository, never the Dexie singleton), which is now `no-restricted-imports`-enforced in `eslint.config.js` rather than left to an honour system. It does not reduce the reactivity coupling described below.
- **Dexie no longer leaks into the UI:** a content grep for `\bdb\.` outside `src/db/` (excluding tests and the infra modules below) finds zero direct table access — every former leak site (`SettingsRoute.tsx`, `TimelineRoute.tsx`, `TemplatesRoute.tsx`, `ManuscriptRoute.tsx`, `BookRoute.tsx`, `MapRoute.tsx`, the manuscript views, `usePage.ts`, etc.) now goes through a repository, and lint fails any new one. The remaining direct `db.*` users are the five infra modules carved out of the lint ban on purpose (`src/backup.ts`, `src/searchSync.ts`, `src/snapshots.ts`, `src/htmlExport.ts`, `src/manuscriptExport.ts` — whole-DB, cross-table work a per-table repository would serve worse) plus `src/db/**` itself. Additionally **`useLiveQuery` appears 77 times across 33 non-test files** — reactivity, not just data access, is coupled to Dexie.
...
2. **Direct `db.*` table access in UI** — DONE (#186). The repository seam now covers all five data groups (pages, maps, page types, calendars/events, manuscript) and `eslint.config.js`'s `no-restricted-imports` bans importing the Dexie `db` singleton outside `src/db/**` and a short, deliberate infra allowlist (`src/backup.ts`, `src/searchSync.ts`, `src/snapshots.ts`, `src/htmlExport.ts`, `src/manuscriptExport.ts`). A content grep for `\bdb\.` outside those paths (excluding tests) now finds zero hits — the sweep is finished and lint keeps it that way, so this is no longer a layer of coupling worth counting.
3. **Reactivity** — the hard one, and the only one of the three that remains. `useLiveQuery` (dexie-react-hooks) appears **77 times across 33 non-test files**, plus **5 raw `liveQuery(` subscriptions** (`db/schema.ts` for the category-colour cache, and four in `src/searchSync.ts` — the search-index sync extracted from `App.tsx`, which no longer holds one itself). Dexie's change tracking is what makes every view update on every edit with zero app code. The repositories deliberately exploit this: `src/db/repositories.ts`'s header notes `useLiveQuery(() => pageRepo.get(id))` stays reactive because Dexie tracks the read globally regardless of how deep in the call stack it happens. **Any storage backend that isn't Dexie must bring its own invalidation story for all ~77 sites.** That is the dominant cost of options (b) and (c) below, and no repository interface — including the now-complete one from #186 — reduces it; the seam was never meant to.
...
`<app-data>/worlds/<loreId>.lore` written atomically (temp file, rename over) on a quiet-window + interval-floor + staleness-ceiling cadence and flushed on close; registry mirrored to `registry.json`; deletion moves the `.lore` to `worlds/trash/`; a launch that finds worlds on disk the registry DB doesn't know about offers to restore them. *Exit criterion met in design;* the end-to-end check (delete the WebView2 profile, relaunch, restore) is a real-filesystem test and lives in `docs/world-mirror-manual-verification.md`.

Three things landed differently from this plan, each for a reason worth keeping:
- **Not the `maybeTakeSnapshot` cadence.** That hook fires only on page saves from `PageRoute`, so map-only, timeline-only and manuscript-only sessions would never have marked the world dirty. The mirror polls instead, so there is no dirty flag and no edit path that can forget to opt in.

  **What it polls, precisely** — an earlier revision of this section said `latestChangeTime()`, "six indexed boundary reads that see every table". Both halves were wrong, in the reassuring direction. `latestChangeTime()` (`src/backup.ts`) reads an indexed `updatedAt`/`createdAt` boundary on **6 of the 15 tables `exportAll()` writes**; the other eleven (pins, regions, templates, docLinks, books, chapters, plotlines, beats, meta, relationshipTypes, relationships) carry no timestamp field at all, so a pin-only, plotline-only, settings-only or relationship-only session left it flat. It stays as-is because `BackupBanner` and `backupOnExit` depend on exactly that shape. The mirror therefore polls its own `mirrorChangeTime()` (`src/worldMirrorSync.ts`), which combines those six reads with a `count()` on each of the other eleven, so an add or a delete registers even with no timestamp to read.

  **It is still not complete, and the gap is by construction:** an in-place edit to a row on those eleven tables is invisible between polls, and `maps`/`calendars` notice an add but not an edit (their `createdAt` index doesn't move). [#233](https://github.com/KantikPotatoe/Lore-Codex/issues/233) is closed: a `MIRROR_MAX_STALE_MS` ceiling (10 minutes) now overrides the quiet window once changes have gone unmirrored that long — never the interval floor, which is checked on every path — so a steady typist's worst-case unmirrored window is bounded at ~10.5 minutes (the ceiling plus one poll interval) instead of the whole session. `flushWorldMirror()`'s unconditional write on close is still the deliberate backstop for the two blind spots above, but it is no longer what bounds a long session — the ceiling is.
